...
./b2 cflags=-fPIC cxxflags=-fPIC link=static threading=multi variant=release runtime-link=shared -j$(($(nproc) + 1))
./b2 cflags=-fPIC cxxflags=-fPIC link=static threading=multi variant=release runtime-link=shared install -j$(($(nproc) + 1)) -d0
cd "${0%/*}"
