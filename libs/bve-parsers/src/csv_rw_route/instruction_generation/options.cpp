...
namespace bve::parsers::csv_rw_route::instruction_generation {
	Instruction create_instruction_options_unitoflength(const line_splitting::InstructionInfo& inst) {
		...
		if (!inst.args.empty()) {
			uol.factors_in_meters.emplace_back(util::parsers::parse_loose_float(inst.args[0], 1));
	...
	Instruction create_instruction_options_unitofspeed(const line_splitting::InstructionInfo& inst) {
	...
	Instruction create_instruction_options_blocklength(const line_splitting::InstructionInfo& inst) {
	...
	Instruction create_instruction_options_objectvisibility(const line_splitting::InstructionInfo& inst) {
		instructions::options::ObjectVisibility ov;

		if (!inst.args.empty()) {
			auto const value = util::parsers::parse_loose_integer(inst.args[0], 0);
	...
	Instruction create_instruction_options_sectionbehavior(const line_splitting::InstructionInfo& inst) {
	...
	Instruction create_instruction_options_cantbehavior(const line_splitting::InstructionInfo& inst) {
	...
	Instruction create_instruction_options_fogbehavior(const line_splitting::InstructionInfo& inst) {
		...
		if (!inst.args.empty()) {
			...
			fb.mode =
			    value == 0 ? instructions::options::FogBehavior::Mode::block_based : instructions::options::FogBehavior::Mode::interpolated;
		}
		else {
	...
	Instruction create_instruction_options_compatibletransparencymode(const line_splitting::InstructionInfo& inst) {
	...
	Instruction create_instruction_options_enablebvetshacks(const line_splitting::InstructionInfo& inst) {
		...
		if (!inst.args.empty()) {
			...
			ts_hacks.mode =
			    value == 0 ? instructions::options::EnableBveTsHacks::Mode::off : instructions::options::EnableBveTsHacks::Mode::on;
		}
		else {
