...
namespace csv_rw_route {
	float pass3_executor::ground_height_at(float position) {
		...
		// *OVERLOADS* GAAH!
		start_iter =
		    std::lower_bound(_route_data.ground_height.begin(), _route_data.ground_height.end(),
		                     position,
		                     [](const ground_height_info& a, float b) { return a.position < b; });
		end_iter =
		    std::upper_bound(_route_data.ground_height.begin(), _route_data.ground_height.end(),
		                     position,
	...
	openbve2::math::evaulate_curve_t pass3_executor::track_position_at(float position) {
		...
		if (position < _route_data.blocks.front().position) {
			...
			starting_it =
			    std::upper_bound(_route_data.blocks.begin(), _route_data.blocks.end(), position,
