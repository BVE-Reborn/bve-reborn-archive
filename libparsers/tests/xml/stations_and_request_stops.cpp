...
TEST_CASE("libparser - xml - stations and request stops - probabilities and messages can have node, early, ontime, late"){
    std::string test_value =
            "<RequestStop>"
            "	<Probability>"
            "       <Early>10</Early>"
...
TEST_CASE("libparser - xml - stations and request stops - early, ontime and late probabilites should add errors"){
    std::string test_value =
            "<RequestStop>"
            "	<Probability>"
            "       <Early>-1</Early>"
...
TEST_CASE("libparser - xml - stations and request stops - stations can contain request stops"){
    ...
    CHECK_EQ(station_marker.request_stop.ai_behaviour, st::request_stop_marker::behaviour::fullspeed);
}

TEST_CASE("libparsers - xml - stations and request stops - stations with request stops should add errors"){
    std::string test_value =
            "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
            "<openBVE xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">"
            "   <Station>"
    ...
    CHECK_EQ(station_marker.request_stop.ai_behaviour, st::request_stop_marker::behaviour::normalbrake);
}