##########
# config #
##########

if (APPLE)
    set(Boost_USE_STATIC_LIBS ON)
else()
    set(Boost_USE_STATIC_LIBS Off)
endif()

find_package(Boost 1.44 REQUIRED COMPONENTS filesystem regex)
find_package(Threads REQUIRED)

################
# Boost Config #
################

if (MSVC)
    if (CMAKE_BUILD_TYPE STREQUAL "Debug")
        set(Boost_USE_DEBUG_RUNTIME On)
    else ()
        set(Boost_USE_DEBUG_RUNTIME Off)
    endif()
    file(GLOB BOOST_DEBUG_BINARY_FILES "${Boost_LIBRARY_DIRS}/*-mt-gd-${Boost_LIB_VERSION}.dll")
    file(GLOB BOOST_BINARY_FILES "${Boost_LIBRARY_DIRS}/*-mt-${Boost_LIB_VERSION}.dll")

    set_property(TARGET Boost::system APPEND PROPERTY
        INTERFACE_COMPILE_DEFINITIONS "BOOST_ALL_DYN_LINK"
    )

    set_property(TARGET Boost::filesystem APPEND PROPERTY
        INTERFACE_COMPILE_DEFINITIONS "BOOST_ALL_DYN_LINK"
    )

    set_property(TARGET Boost::regex APPEND PROPERTY
        INTERFACE_COMPILE_DEFINITIONS "BOOST_ALL_DYN_LINK"
    )

	if (${CMAKE_GENERATOR} MATCHES "Visual Studio|Xcode")
		foreach(config ${CMAKE_CONFIGURATION_TYPES})
			if(${config} MATCHES "Debug")
				file(COPY ${BOOST_DEBUG_BINARY_FILES} DESTINATION ${CMAKE_BINARY_DIR}/bin/${config})
			else()
				file(COPY ${BOOST_BINARY_FILES} DESTINATION ${CMAKE_BINARY_DIR}/bin/${config})
			endif()
		endforeach()
	else()
		file(COPY ${BOOST_BINARY_FILES} DESTINATION ${CMAKE_BINARY_DIR}/bin)
	endif()
endif()
